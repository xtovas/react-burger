    ...
    render() {
      return (
        <div className={ConstructorStyles.orderContainer}>
          {this.state.bun[0] ? (
            <BurgerElement item={this.state.bun[0]} type="top" />
          ) : (
            ""
          )}
          <ul className={ConstructorStyles.orderContent}>{this.state.items}</ul>
          {this.state.bun[0] ? (
            <BurgerElement item={this.state.bun[0]} type="bottom" />
          ) : (
            ""
          )}
